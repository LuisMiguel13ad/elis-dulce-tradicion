        ...
        <div style="background: linear-gradient(135deg, #d4af37 0%, #f4d03f 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: #fff; margin: 0; font-size: 28px;">&#127874; ${labels.title}</h1>
                ...
                <div style="background: #fff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
                    <p style="font-size: 16px;">${labels.greeting} ${safeName},</p>
                        <p style="font-size: 16px;">${labels.intro}</p>

                            <div style="background: #f9f9f9; padding: 20px; border-radius: 8px; margin: 20px 0;">
                                  <h2 style="color: #d4af37; margin-top: 0;">${labels.detailsHeader}</h2>
                                        <p><strong>${labels.orderNumber}</strong> ${safeOrderNumber}</p>
                                              <p><strong>${labels.dateNeeded}</strong> ${formatDate(order.date_needed, lang)} ${labels.at} ${escapeHtml(order.time_needed)}</p>
                                                    <p><strong>${labels.size}</strong> ${safeCakeSize}</p>
                                                          <p><strong>${labels.filling}</strong> ${safeFilling}</p>
                                                                <p><strong>${labels.theme}</strong> ${safeTheme}</p>
                                                                      ${safeDedication ? `<p><strong>${labels.dedication}</strong> "${safeDedication}"</p>` : ''}
                                                                            <p><strong>${labels.delivery}</strong> ${order.delivery_option === 'delivery' ? labels.deliveryHome : labels.deliveryPickup}</p>
                                                                                  ${safeDeliveryAddress ? `<p><strong>${labels.address}</strong> ${safeDeliveryAddress}${safeDeliveryApartment ? `, ${safeDeliveryApartment}` : ''}</p>` : ''}
