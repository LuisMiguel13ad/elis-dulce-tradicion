 ...
 * @returns {Promise<{success: boolean, error?: string, messageId?: string, skipped?: boolean, reason?: string}>}
 */
 ...
 * @param {any} order 
 */
export async function sendReadyNotificationEmail(order) {
    return callEdgeFunction('send-ready-notification', { order });
}

/**
 * Send failed payment notification email to admin
 * @param {{amount: number, customer_name: string, customer_email: string, error_message: string, idempotency_key?: string}} payment 
 */
 ...
 * @param {'order_updates' | 'ready_notifications' | 'promotions'} notificationType 
 */
